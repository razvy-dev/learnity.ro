  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      <main className="flex-grow">
        ...
        <section ref={impactRef} className="py-20 px-4 bg-customLightBlue bg-opacity-10">
          <div className="max-w-6xl mx-auto">
            <div
              className={`mb-16 text-center ${impactInView ? "animate-slide-up" : "opacity-0"}`}
              style={{ transitionDelay: "0.1s" }}
            >
              <h2 className="text-4xl md:text-5xl font-bangers text-customBlack mb-4">Poveștile Noastre de Impact</h2>
              <div className="w-32 h-2 bg-customOrange mx-auto rounded-full"></div>
              <p className="text-lg text-customBlack max-w-3xl mx-auto mt-6">
                Descoperă cum donațiile transformă vieți prin educație și creează oportunități pentru tinerii din
                România.
              </p>
            </div>

            <div className={`${impactInView ? "animate-fade-in" : "opacity-0"}`} style={{ transitionDelay: "0.2s" }}>
              <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
                <div className="grid grid-cols-1 md:grid-cols-2">
                  {/* Image side */}
                  <div className="relative h-full">
                    <img
                      src={impactStories[activeImpactStory].image || "/placeholder.svg"}
                      alt={`Povestea lui ${impactStories[activeImpactStory].name}`}
                      className="w-full h-full object-cover"
                    />
                    <div className="absolute inset-0 bg-gradient-to-r from-black to-transparent opacity-50"></div>
                    <div className="absolute bottom-0 left-0 p-6 text-white">
                      <p className="text-2xl font-bold">{impactStories[activeImpactStory].name}</p>
                      <p>{impactStories[activeImpactStory].age} ani</p>
                    </div>
                  </div>
                  ...
                  <div className="p-8 flex flex-col justify-center">
                    <div className="mb-6">
                      <svg
                        className="w-12 h-12 text-customOrange opacity-20"
                        fill="currentColor"
                    ...
                    <p className="text-xl text-customBlack italic mb-8">{impactStories[activeImpactStory].quote}</p>
                    ...
                    <div className="flex justify-between items-center mt-auto">
                      <div className="flex space-x-2">
                        {impactStories.map((_, index) => (
                          <button
                            key={index}
                            onClick={() => setActiveImpactStory(index)}
                            className={`w-3 h-3 rounded-full transition-all duration-300 ${
                              activeImpactStory === index ? "bg-customBlue w-8" : "bg-gray-300"
                            }`}
                            aria-label={`Vezi povestea ${index + 1}`}
                          />
                        ))}
                      </div>
                      ...
                      <Link
                        to="/impact-stories"
                        className="text-customBlue hover:text-customOrange transition-colors duration-300 font-bold flex items-center"
                      >
